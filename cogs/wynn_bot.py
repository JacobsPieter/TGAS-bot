...
- Implements periodic tasks for data synchronization
- Integrates with a local database for historical data tracking
- Generates formatted images for raid statistics display

Dependencies:
- discord.py (with app_commands extension)
- requests (for API communication)
- python-dotenv (for environment variable management)
- database.py (local data persistence)
- markdown_card.py (formatted statistics display)

Configuration:
- Requires BOT_TOKEN environment variable
- Configurable API query intervals
- Font configuration for markdown card generation
...
def get_member_stats(entered_guild_data: dict) -> dict:
    ...
    Raises:
        KeyError: If required data fields are missing from the API response
    """
    members = {}
...
def get_player_guild_raids(player: str, member_stats) -> dict[str, int]:
    ...
    Raises:
        KeyError: If the player is not found in member statistics
    """
    stats = member_stats
...
def get_latest_graid_completions(uuid) -> dict[str, int]:
    ...
    Raises:
        database.DatabaseError: If there's an issue querying the database
    """
...
class Info(discord.Client):
    ...
    def __init__(self):
        ...
        Sets up the bot with message content intent enabled and initializes
        the command tree for slash command handling.
        """
    ...
    async def setup_hook(self):
        ...
        This method is called after the bot is ready and is used to
        synchronize the command tree with Discord's servers.
        """
...
async def guild_api_query(guild_prefix):
    ...
    Raises:
        requests.RequestException: If the API request fails
        database.DatabaseError: If there's an issue updating the database
    """
...
async def check_graid_completions(channel_id: int):
    ...
    Raises:
        discord.DiscordException: If there's an issue sending the message
        database.DatabaseError: If there's an issue updating raid statistics
    """
    error_messages: str = ''
...
async def xp_contributions(timespan, channel_id):
    ...
    Raises:
        discord.DiscordException: If there's an issue sending the message
        database.DatabaseError: If there's an issue updating contribution data
    """
    contributed_deltas = {}
    for player, data in guild_members.items():
...
@client.tree.command(
        name="start_tracking_guild_raids",
...
async def track_xp_contributions(interaction: discord.Interaction):
    ...
    Raises:
        discord.DiscordException: If there's an issue sending the response
        database.DatabaseError: If there's an issue initializing contribution data
    """
    await interaction.response.defer()
    target_channel_id = interaction.channel_id
    if target_channel_id is None:
...
async def raids(interaction: discord.Interaction):
    ...
    Raises:
        discord.DiscordException: If there's an issue sending the response
        FileNotFoundError: If the generated image file cannot be found
    """
    raid_data = {}
    await interaction.response.defer()
    for player, data in guild_members.items():
...
async def on_message(message):
    ...
    encouraged_words = ['gamble', 'gamba', 'roll', 'gambling', 'gambler']
